...
# @Desc      :   ruyisdk gnu-upstream and llvm-upstream test
# #############################################
...
function run_test() {
    ...
    ruyi venv -t llvm-upstream --sysroot-from gnu-upstream $qemu_cmd generic ./llvm-venv
    CHECK_RESULT $? 0 0 "Check ruyi venv creation for llvm-upstream with gnu-upstream failed"

    . llvm-venv/bin/ruyi-activate
